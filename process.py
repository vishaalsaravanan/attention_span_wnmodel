...
def attention():
    ...
    while(timer_run and ttt!=30):
        ...
        df=pd.DataFrame()
        df['Time'].append(ttt)
        df['Blink_Count'].append(b3)
        df['Pixel'].append(ssim)
        df['Emotion'].append(exp)
        df['Looking'].append(distract_mean)
        df['Noise'].append(rn)
        df['Face'].append(face)
...
def fun(value):
    print(value)
    render_template('index.html',prediction_text='Value is {}'.format(value))
